...
export function splitDocument(doc: string): {
...
export const frontmatterField = StateField.define<FrontmatterInfo>({
  ...
  update: (value, tr) =>
    tr.docChanged ? parseFrontmatter(tr.newDoc.toString()) : value,
});

export const wordCountField = StateField.define<number>({
  create: (state) => countWords(state.doc.toString()),
  update: (value, tr) =>
    tr.docChanged ? countWords(tr.newDoc.toString()) : value,
});
