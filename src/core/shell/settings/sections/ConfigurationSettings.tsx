    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant="secondary" className="gap-1.5">
          <badge.icon
            className={`w-3.5 h-3.5 ${badge.spin ? "animate-spin" : ""}`}
          />
          {badge.text}
        </Badge>
        {state.lastSyncedAt && (
      ...
      <p className="text-xs text-muted-foreground">
        Your graph styling, layout engine, theme, editor, workspace, features
        and schema are saved on this device and, when you are signed in, to your
        account — so they come back after a refresh.
      </p>
        ...
        {sections.map((section) => (
          ...
          >
            ...
            <span className="text-[11px] uppercase tracking-wide text-muted-foreground">
              {section.scope === "vault" ? "Per vault" : "Account-wide"} · v
              {section.version}
            </span>
          </div>
        ))}
