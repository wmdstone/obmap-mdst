...
import { extractMentions } from "@/core/system/metadata/markdown-parser";
import {
  enrichNode,
  metadataCache,
} from "@/core/system/metadata/MetadataCache";
import {
  linkMentions,
  renameNode,
} from "@/core/system/file-manager/FileManager";
import {
  useIndexedNodes,
  useMetadataVersion,
...
import {
  ...
  writeAllNodesToDisk,
  writeNodeToDisk,
} from "@/core/system/persistence/VaultFileWriter";
import { isFeatureEnabled } from "@/core/system/plugins/feature-toggles";
import { useFeatureEnabled } from "@/core/system/plugins/useFeatureEnabled";
import { useAutoLinks } from "@/core/graph/hooks/useAutoLinks";
...
export function VaultSessionProvider({ children }: { children: ReactNode }) {
  ...
  const setGraphData = useCallback(
    (data: GraphData) => setNodes(data.nodes),
    [setNodes],
  );
  ...
  const loadActiveVault = useCallback(() => {
    ...
    if (activeVault) {
      ...
      setVaultGraphConfig(savedGraphConfig as never);
      loadConfig(savedGraphConfig as never);
  ...
  // Switching vaults resets the session: notes, graph config and the window
  // layout all come from the newly opened vault.
  useEffect(() => {
    const unsubscribe = eventBus.subscribe(EventType.VAULT_SWITCHED, () => {
      setSelectedNode(null);
      resetNodes();
      loadActiveVault();
    });
    return unsubscribe;
  }, [loadActiveVault, resetNodes, setSelectedNode]);
  ...
  const onSwitchVault = useCallback(
    async (vaultId: string) => {
      ...
      const nextVault = vaultManager.getVault(vaultId);
      if (!nextVault) {
        toast.error("Vault is no longer available");
        return;
      }

      const switched = await vaultManager.switchVault(vaultId);
      if (!switched) {
  ...
  const commitNodes = useCallback(
    async (nextNodes: Node[]) => {
      ...
      await writeAllNodesToDisk(vault, nextNodes);
      await saveVault();
  ...
  const onNodeUpdate = useCallback(
    async (incomingNode: Node) => {
      ...
      if (
        ...
        previous.name !== updatedNode.name
      ) {
        ...
        const renameVault = currentVaultId
          ? vaultManager.getVault(currentVaultId)
          : null;
  ...
  const onLinkMention = useCallback(
    async (sourceNodeId: string, targetName: string) => {
      ...
      const updated = enrichNode(
        { ...source, content },
        metadataCache.parse(content),
      );
      await commitNodes(
        nodes.map((n) => (n.id === sourceNodeId ? updated : n)),
      );
    ...
    async (nodeId: string) => {
      ...
      if (currentVaultId) {
        ...
        if (vault) {
          ...
          await deleteNodeFromDisk(vault, nodesBefore, nodeId);
          vault.history.addState(
    ...
    async (nodeId: string, newParentId: string | null) => {
      ...
      if (currentVaultId) {
        ...
        if (vault) {
          ...
          await moveNodeOnDisk(vault, nodesBeforeMove, updatedNodes, nodeId);
  ...
  const onAddNode = useCallback(
    async (type: "folder" | "file") => {
      // Folder selected -> create inside it. File/media selected -> create as a sibling
      // inside the same parent folder. Nothing selected -> create at root.
      ...
      if (currentVaultId) {
        ...
        if (vault) {
          ...
          await writeNodeToDisk(vault, [...nodes, newNode], newNode.id);
    ...
    async (importedNodes: Node[], updatedNodes?: Node[]) => {
      ...
      if (currentVaultId) {
        ...
        if (vault) {
          ...
          await writeAllNodesToDisk(vault, result);
  ...
  const availableVaults = vaultManager.getAllVaults().map((item) => ({
    id: item.id,
    name: item.name,
    location: item.location,
    nodeCount: item.graphService.getGraphData().nodes.length,
  }));
