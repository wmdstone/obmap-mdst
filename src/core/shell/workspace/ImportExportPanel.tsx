...
import {
  ...
  GraphNode,
  DuplicateStrategy,
} from "@/core/system/import-export/services/ImportExportService";
...
export const ImportExportPanel = ({
  ...
  const [selectedExportNodes, setSelectedExportNodes] = useState<Set<string>>(
    new Set(),
  );
  ...
  const handleFileImport = async (files: FileList | null, type: string) => {
    ...
    try {
      ...
      const result = await importExportService.importFiles(
        files,
        nodesRef.current as GraphNode[],
        duplicateStrategy,
      );
  ...
  const handleExport = async (type: ExportOptions["type"]) => {
    const options: ExportOptions = {
      ...
      includeMedia,
      nodeIds:
        type === "folder" || type === "files"
          ? Array.from(selectedExportNodes)
          : undefined,
    };

    if (
      (type === "folder" || type === "files") &&
      selectedExportNodes.size === 0
    ) {
    ...
    <Tabs defaultValue="import" className="w-full">
      ...
      <TabsContent value="import" className="space-y-4 mt-4">
        ...
        <Collapsible
          ...
          onOpenChange={setShowImportSettings}
        >
          <CollapsibleTrigger asChild>
            ...
            >
              ...
              <ChevronDown
                className={`w-4 h-4 transition-transform ${showImportSettings ? "rotate-180" : ""}`}
              />
            </Button>
          </CollapsibleTrigger>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary">
              {selectedExportNodes.size} selected
            </Badge>
          </div>
