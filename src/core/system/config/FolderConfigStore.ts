...
import type { FileSystemService } from "@/core/system/persistence/FileSystemService";
import type { SectionRecord } from "./types";

export class FolderConfigStore {
  private async service(): Promise<FileSystemService | null> {
    try {
      return await container.resolve<FileSystemService>(
  ...
  async writeSections(
    entries: Array<{ id: string; record: SectionRecord }>,
  ): Promise<boolean> {
