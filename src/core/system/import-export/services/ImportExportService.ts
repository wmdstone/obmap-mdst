...
export class ImportExportService {
  ...
  private getFileType(
    filename: string,
  ): "markdown" | "image" | "audio" | "video" | "unknown" {
  ...
  private findExistingNode(
    ...
    return nodes.find(
      (n) => n.name === name && n.parentId === parentId && n.type === type,
    );
  }
  ...
  async importFiles(
    ...
    const existingRoot = existingNodes.find(
    ...
    for (const file of Array.from(files)) {
      try {
        if (file.size > MAX_FILE_SIZE) {
          result.errors.push(
            `File too large: ${file.name} (max ${MAX_FILE_SIZE / 1024 / 1024}MB)`,
          );
          continue;
        }
        ...
        if (file.name.endsWith(".zip")) {
          const zipResult = await this.importZip(
            file,
        ...
        for (let i = 0; i < pathParts.length - 1; i++) {
          ...
          if (!folderMap.has(folderPath)) {
            ...
            const folderDepth = currentParentId
              ? (allCurrentNodes.find((n) => n.id === currentParentId)?.depth ??
                  -1) + 1
              : 0;
        ...
        const parentId = parentPath
          ? folderMap.get(parentPath) || importRootId
          : importRootId;
        ...
        if (fileType === "markdown") {
          ...
          const existingFile = this.findExistingNode(
            allNodes,
            fileNameWithoutExt,
            parentId,
            "file",
          ...
          const uniqueName =
            existingFile && duplicateStrategy === "rename"
          ...
          const existingMedia = this.findExistingNode(
            allNodes,
            fileNameWithoutExt,
            parentId,
            "media",
          ...
          const uniqueName =
            existingMedia && duplicateStrategy === "rename"
              ? this.generateUniqueName(allNodes, fileNameWithoutExt, parentId)
              : fileNameWithoutExt;
                ...
                switch (duplicateStrategy) {
                  ...
                  case "merge":
                    updatedNodes.push({
                      ...
                      content: existingFile.content + "\n\n---\n\n" + content,
                      tags: [
                        ...new Set([...existingFile.tags, ...parsed.tags]),
                      ],
                      wikilinks: [
                        ...new Set([
                          ...(existingFile.wikilinks || []),
   ...
   */
  async exportData(nodes: GraphNode[], options: ExportOptions): Promise<Blob> {
    const zip = new JSZip();
    ...
    if (options.type !== "media" && !options.includeMedia) {
      nodesToExport = nodesToExport.filter(
        (n) => n.type !== "media" && !n.mediaType,
      );
    }
