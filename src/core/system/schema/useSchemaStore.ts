  ...
  persist(
    (set, get) => ({
      ...
      setStrictMode: (strictMode) =>
        set((s) => ({ schema: { ...s.schema, strictMode } })),
      ...
      updateProperty: (key, updates) =>
        set((s) => ({
          schema: {
            ...s.schema,
            properties: s.schema.properties.map((p) =>
      ...
      removeProperty: (key) =>
        set((s) => ({
          schema: {
            ...s.schema,
            properties: s.schema.properties.filter((p) => p.key !== key),
          },
        })),
      ...
      persistToVault: async (vaultId) => {
        try {
          ...
          await manager.setGraphConfig(vaultId, {
            ...config,
            schema: get().schema,
          });
        } catch {
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        return {
          ...current,
          schema: p?.schema ? normalizeSchema(p.schema) : current.schema,
        };
      },
    },
  ),
