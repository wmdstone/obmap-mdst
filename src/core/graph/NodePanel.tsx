...
const MediaPreview = ({ node }: { node: Node }) => {
  ...
  const togglePlay = () => {
    const media =
      node.mediaType === "audio" ? audioRef.current : videoRef.current;
    if (media) {
      if (isPlaying) {
        media.pause();
      } else {
...
export const NodePanel = ({
  ...
  useEffect(() => {
    if (node && lastNodeIdRef.current === node.id) {
      ...
      const hasTagsChange =
        JSON.stringify(tags) !== JSON.stringify(node.tags || []);
  ...
  const copyEmbedLink = () => {
    const embedSyntax =
      node.mediaType === "image" ? `![[${node.name}]]` : `[[${node.name}]]`;
      ...
      <ScrollArea className="flex-1 scrollbar-thin scrollbar-thumb-muted-foreground/20 scrollbar-track-transparent">
        ...
        >
          ...
          <Input
            ...
            placeholder={
              node.type === "folder" ? "Untitled folder" : "Untitled"
            }
